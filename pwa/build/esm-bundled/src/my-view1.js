import{PolymerElement,html}from"./my-app.js";const imageBaseUrl="https://picsum.photos/200?random";class RandomImage extends PolymerElement{constructor(){super();this.imageUrl=imageBaseUrl}static get template(){return html`
            <img src$={{imageUrl}} alt="A random image"/>
        `}static get properties(){return{refreshFrequency:{type:Number,value:5e3}}}ready(){super.ready();this.setTimer()}setTimer(){setInterval(_=>{this.imageUrl=imageBaseUrl+"&cache_prevention="+Math.floor(1e3*Math.random())},this.refreshFrequency)}}customElements.define("random-image",RandomImage);const imageBaseUrl$1="https://picsum.photos/200?random",refreshFrequency=5e3;class RandomProcessedImage extends PolymerElement{static get template(){return html`
...
}return data}function truncateColor(value){if(0>value){value=0}else if(255<value){value=255}return value}function applyContrast(data,contrast){for(var factor=259*(contrast+255)/(255*(259-contrast)),i=0;i<data.length;i+=4){data[i]=truncateColor(factor*(data[i]-128)+128);data[i+1]=truncateColor(factor*(data[i+1]-128)+128);data[i+2]=truncateColor(factor*(data[i+2]-128)+128)}}class MyView1 extends PolymerElement{static get template(){return html`
